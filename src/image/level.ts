...
export class Level {
  ...
  public setNickname(nickname: string, color?: string, font?: string): this {
    if (typeof nickname !== "string") {
      throw new Error(
        `El apodo /*nickname/* no está ingresado en tipo string, en su lugar se ingresó: ${typeof nickname}`,
      );
    }
  ...
  public setRank(
    ...
    if (typeof rank !== "number" && typeof rank !== "string") {
      throw new Error(
        `El rank no está ingresado en tipo number o string, en su lugar se ingresó: ${typeof rank}`,
      );
    }
    this.rank = rank;
  ...
  public setLevel(
    ...
    if (typeof level !== "number") {
      throw new Error(
        `El nivel no está ingresado en tipo number, en su lugar se ingresó: ${typeof level}`,
      );
    }
    this.level = level;
  ...
  public setXp(
    ...
    if (typeof xp !== "number") {
      throw new Error(
        `El xp no está ingresado en tipo number, en su lugar se ingresó: ${typeof xp}`,
      );
    }
    if (typeof max !== "number") {
      throw new Error(
        `El maximo de xp no está ingresado en tipo number, en su lugar se ingresó: ${typeof max}`,
      );
    }

    if (xp > max) {
      throw new Error(
        `La experiencia actual no puede ser mayor a la experiencia requerida, ${xp} > ${max}`,
      );
    }
    ...
    if (color) {
      if (typeof color !== "string") {
        throw new Error(
          `El color del xp no está ingresado en tipo string, en su lugar se ingresó: ${typeof color}`,
        );
      }
    ...
    if (text) {
      if (typeof text !== "string") {
        throw new Error(
          `El texto del xp no está ingresado en tipo string, en su lugar se ingresó: ${typeof text}`,
        );
      }
      this.xpText = text;
    }
    if (font) {
      if (typeof font !== "string") {
        throw new Error(
          `La fuente del xp no está ingresado en tipo string, en su lugar se ingresó: ${typeof font}`,
        );
      }
  ...
  public setCurrentBarColor(
    color: string | { hex: string; position: number }[],
  ): this {
    if (typeof color !== "string" && !Array.isArray(color)) {
      throw new Error(
        `El color de la barra no está ingresado en tipo string o array, en su lugar se ingresó: ${typeof color}`,
      );
    }
  ...
  public async render(): Promise<Buffer> {
    ...
    if (typeof this.colors.bar === "object") {
      ...
      for (let i = 0; i < this.colors.bar.length; i++) {
        gradient.addColorStop(
          this.colors.bar[i].position,
          this.colors.bar[i].hex,
        );
      }
    ...
    ctx.fillText(
      xpText
        ...
        .replace(/{max}/g, abbreviateNumber(this.xp.max)),
      965,
      230,
      400,
    );
